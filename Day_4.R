...
ggplot(data = SACTN1, aes(x = date, y = temp)) +
  geom_line(aes(colour = site, group = paste0(site, src))) + # paste0 
  labs(x = "", y = "Temperature (°C)", colour = "Site") +
  theme_bw()

# paste0("Port Nolloth", "DEA")
...
# SACTN3 has depth and type in 1 column called var - should sep into 2 sep cols cos vars must bein cols
SACTN3_tidy1 <- SACTN3 %>% 
...
# seps index col into c(c= col?) 2 where it is sepd in the col by "\ " 
